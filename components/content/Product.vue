        ...
        <div class="lg:grid lg:grid-cols-2 lg:items-start lg:gap-x-8">
          ...
          <TabGroup as="div" class="flex flex-col-reverse">
            ...
            <div class="mx-auto mt-6 hidden w-full max-w-2xl sm:block lg:max-w-none">
              <TabList class="grid grid-cols-4 gap-6">
                <Tab v-for="image in product.images" :key="image.id" class="relative flex h-24 cursor-pointer items-center justify-center rounded-md bg-white text-sm font-medium uppercase text-gray-900 dark:text-gray-100 hover:bg-gray-50 focus:outline-none focus:ring focus:ring-opacity-50 focus:ring-offset-4" v-slot="{ selected }">
          ...
          <div class="mt-10 px-4 sm:mt-16 sm:px-0 lg:mt-0">
            ...
            <div class="mt-3">
              ...
              <p class="text-3xl tracking-tight text-gray-900 dark:text-gray-100">{{ product.price }}</p>
            </div>
            ...
            <div class="mt-6">
              ...
              <div class="space-y-6 text-base text-gray-700  dark:text-gray-100" v-html="product.description" />
            </div>
  
            <form class="mt-6">
              <!-- Colors -->
              <div>
                <h3 class="text-sm text-gray-900 dark:text-gray-100">Color</h3>
              ...
              <div class="mt-10 flex">
                <NuxtLink :to="localePath('/product')" class="flex max-w-xs flex-1 items-center justify-center rounded-md border border-transparent bg-indigo-600 px-8 py-3 text-base font-medium text-white hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 focus:ring-offset-gray-50 sm:w-full">Add to bag</NuxtLink>
  
                <button type="button" class="ml-4 flex items-center justify-center rounded-md px-3 py-3 text-gray-400  dark:text-gray-100 hover:bg-gray-100 hover:text-gray-500">
                ...
                <Disclosure as="div" v-for="detail in product.details" :key="detail.name" v-slot="{ open }">
                  <h3>
                    <DisclosureButton class="group relative flex w-full items-center justify-between py-6 text-left">
                      <span :class="[open ? 'text-indigo-600' : 'text-gray-900 dark:text-gray-100', 'text-sm font-medium']">{{ detail.name }}</span>
                      <span class="ml-6 flex items-center">
                        <PlusIcon v-if="!open" class="block h-6 w-6 text-gray-400  dark:text-gray-100 group-hover:text-gray-500" aria-hidden="true" />
                  ...
                  <DisclosurePanel as="div" class="prose prose-sm pb-6">
                    <ul role="list">
                      <li v-for="item in detail.items" :key="item" class="text-gray-900 dark:text-gray-100">{{ item }}</li>
                    </ul>
                  </DisclosurePanel>
                </Disclosure>
              </div>
